...
* 임직원과 비임직원 데이터를 각각 나누어, 모델링을 진행해본 결과, 임직원의 경우 성능이 높게 나왔지만, 비임직원의 경우 성능이 낮게 나온 점을 참고해볼 때, 비임직원에게 많은 영향을 줄 수 있는 변수와 상품, 행사에 대한 정보를 좀 더 파악하고 분석해보면 프라임 회원 예측의 성능을 향상시킬 수 있다.
